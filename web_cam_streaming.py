...
app = Flask(__name__)
capture = cv2.VideoCapture('/dev/video0', cv2.CAP_V4L)  # 웹캠으로부터 비디오 캡처 객체 생성
# capture.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # 캡처된 비디오의 폭 설정
# capture.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # 캡처된 비디오의 높이 설정
capture.set(3, __SCREEN_WIDTH)
capture.set(4, __SCREEN_HEIGHT)
...
@app.route('/')
def Index():
    return render_template('streamIn.html')  # index.html 파일을 렌더링하여 반환합니다.
